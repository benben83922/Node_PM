...
export default function MilestoneTimeline({ milestones }) {
  if (!milestones?.length) return <p className="text-sm text-gray-400">無里程碑資料</p>
      ...
      {sorted.map(m => {
        ...
        return (
          <li key={m.id} className="ml-4">
            <span className={`absolute -left-1.5 mt-1 h-3 w-3 rounded-full border-2 border-white
              ${m.is_completed ? 'bg-green-500' : 'bg-gray-300'}`} />
            <p className="text-sm font-medium text-gray-800">{m.milestone_name}</p>
            <p className="text-xs text-gray-400">{formatDate(m.planned_date)}</p>
            {!m.is_completed && days !== null && (
              <p className={`text-xs mt-0.5 ${days < 0 ? 'text-red-500' : 'text-gray-500'}`}>
                {days < 0 ? `已逾期 ${Math.abs(days)} 天` : `${days} 天後`}
              </p>
            )}
            {m.is_completed && (
